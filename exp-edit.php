...
<div class="container content">
	...
	<?php
		include('conx.php');
			if($stmt = $conn->prepare("SELECT ques.q_id, ques.ques, category.value, answers.a_id, answers.answer FROM ques, category, answers WHERE ques.category=category.c_id AND ques.q_id=answers.q_id AND answers.answer_by=? AND ques.q_id=? AND answers.a_id=? ORDER BY ques.add_date DESC, ques.add_time DESC")){
				if($stmt->bind_param("sii", $_SESSION['exp'], $_GET['q_id'], $_GET['a_id'])){
					$stmt->execute();
					$stmt->bind_result($q_id, $ques, $category, $a_id, $answer);					
					if($stmt->fetch()){						
										...
										<div class="row">
											<form class="col s12 m12 l12" id="answer_form" name="answer_form" autocomplete="off">
												...
												<div class="input-field col s12">
												  <textarea id="answer-box" class="materialize-textarea">'.$answer.'</textarea>
...
<script>
	$(document).on("submit", "#answer_form", function(event){
		...
		var formData={ answer_btn:"answer_btn", ans_id:<?php echo $_GET['a_id']; ?>, ans:ans };		
